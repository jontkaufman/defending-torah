...
// Returns the active course — Course 2 HTML sets window.__ACTIVE_COURSE before render
function getCourse() { return window.__ACTIVE_COURSE || COURSE_DATA; }
...
const COURSE_DATA = {
  ...
  level: "Beginner",
  edition: 'No. 001',
  kicker: 'Foundations Course — Beginner Level',
  hero_title_line1: 'Learn to',
  hero_title_em: 'defend',
  hero_title_line2: 'Torah.',
  hero_lede: "A 5-week, 10-session beginner course for young believers who want to understand Torah as part of God's unchanging instruction — and answer common objections with clarity, humility, and confidence.",
  hero_tags: [['5 Weeks',''], ['10 Sessions',''], ['12 Objections Addressed','crimson'], ['Beginner Level','']],
  watermark: 'תורה',
  syllabus_tagline: 'One consistent story.',
  hero_card: {
    hebrew: 'יֵשׁוּעַ',
    transliteration: 'kept Torah.',
    tagline: 'so should we.',
...
function CourseLanding({ onNav }) {
  ...
  const C = getCourse();
  const week = C.weeks_data[activeWeek - 1];
  const card = C.hero_card || {};
  ...
  return (
    <div>
      ...
      <section style={{ position: 'relative', zIndex: 1, padding: '70px 64px 90px', borderBottom: '1px solid var(--ink)', display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 64, overflow: 'hidden' }}>
        {/* Watermark */}
        <div style={{ position: 'absolute', top: 20, right: 60, fontFamily: "'Frank Ruhl Libre', serif", fontWeight: 900, fontSize: 280, opacity: 0.03, color: 'var(--ink)', lineHeight: 1, pointerEvents: 'none', userSelect: 'none' }}>{C.watermark}</div>
        <div style={{ position: 'absolute', top: 90, right: 40, writingMode: 'vertical-rl', fontFamily: "'JetBrains Mono', monospace", fontSize: 10, letterSpacing: '0.3em', textTransform: 'uppercase', color: 'var(--muted)' }}>{C.edition} — Course Edition</div>
        ...
        <div style={{ position: 'relative' }}>
          ...
          <h1 style={{ fontFamily: "'Frank Ruhl Libre', serif", fontWeight: 300, fontSize: 'clamp(44px, 6vw, 82px)', lineHeight: 0.98, letterSpacing: '-0.03em', color: 'var(--ink)', marginBottom: 28 }}>
            {C.hero_title_line1 || 'Learn to'}<br />
            <em style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: 'italic', fontWeight: 400, color: 'var(--ochre-deep)' }}>{C.hero_title_em || 'defend'}</em> {C.hero_title_line2 || 'Torah.'}
          </h1>
          <p style={{ fontSize: 20, lineHeight: 1.55, maxWidth: 520, color: 'var(--ink-soft)', marginBottom: 16, fontFamily: "'Cormorant Garamond', serif" }}>
            {C.hero_lede}
          </p>
          {C.prerequisite && (
            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: 14 }}>
              <span style={{ border: '1px solid var(--olive)', color: 'var(--olive)', padding: '2px 10px' }}>Prerequisite: {C.prerequisite}</span>
            </div>
          )}
          <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', marginBottom: 40 }}>
            {C.hero_tags.map(([label, col]) => (
        ...
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'flex-start', paddingTop: 20 }}>
          <div style={{ position: 'relative', width: '100%', maxWidth: 420, aspectRatio: '3/4', background: 'var(--parchment-deep)', border: '1px solid var(--ink)', padding: '36px 34px', boxShadow: '18px 18px 0 -1px var(--parchment-deep), 18px 18px 0 0 var(--ink)' }}>
            ...
            <div style={{ fontFamily: "'Frank Ruhl Libre', serif", fontWeight: 300, fontSize: 'clamp(60px, 9vw, 100px)', lineHeight: 0.92, color: 'var(--ink)', textAlign: 'right', marginBottom: 14, letterSpacing: '-0.02em' }} dir="rtl">{card.hebrew}</div>
            <div style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: 'italic', fontSize: 22, color: 'var(--ochre-deep)', textAlign: 'right', marginBottom: 6 }}>{card.transliteration}</div>
            <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 15, color: 'var(--ink-soft)', textAlign: 'right', marginBottom: 24, paddingBottom: 18, borderBottom: '1px solid var(--ink)' }}>{card.tagline}</div>
            <p style={{ fontSize: 15, lineHeight: 1.65, color: 'var(--ink)', fontStyle: 'italic', fontFamily: "'Cormorant Garamond', serif" }}>{card.quote}</p>
            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--crimson)', textAlign: 'right', marginTop: 12 }}>{card.ref}</div>
          </div>
        </div>
      </section>
      ...
      <section id="course-weeks" style={{ position: 'relative', zIndex: 1, padding: '80px 64px 100px', borderBottom: '1px solid var(--ink)' }}>
        <div style={{ marginBottom: 56 }}>
          ...
          <h2 style={{ fontFamily: "'Frank Ruhl Libre', serif", fontWeight: 300, fontSize: 'clamp(34px, 4vw, 54px)', lineHeight: 1.02, letterSpacing: '-0.025em', marginBottom: 0 }}>
            Five weeks. Ten sessions.<br /><em style={{ fontFamily: "'Cormorant Garamond', serif", color: 'var(--ochre-deep)', fontStyle: 'italic', fontWeight: 400 }}>{C.syllabus_tagline}</em>
          </h2>
        </div>
...
function WeekView({ weekNum, onNav }) {
  const C = getCourse();
  const week = C.weeks_data[weekNum - 1];
  if (!week) return null;
    ...
    <div>
      ...
      <section style={{ padding: '60px 64px 70px', borderBottom: '1px solid var(--ink)' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 40, alignItems: 'end', marginBottom: 48 }}>
          <div>
            <MonoLabel color="var(--crimson)" style={{ marginBottom: 16 }}>Week {week.num} of {C.weeks}</MonoLabel>
...
function StudyMethodCard({ method }) {
  return (
    <div style={{ border: '1px solid var(--ink)', background: 'var(--parchment-deep)', marginTop: 24 }}>
      <div style={{ background: 'var(--ink)', padding: '12px 20px' }}>
        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10.5, letterSpacing: '0.28em', textTransform: 'uppercase', color: 'var(--ochre)' }}>Seven-Step Study Method</div>
      </div>
      {method.map((s, i) => (
        <div key={s.step} style={{ display: 'grid', gridTemplateColumns: '52px 1fr', gap: 16, alignItems: 'flex-start', padding: '18px 24px', borderBottom: i < method.length - 1 ? '1px solid var(--parchment-shadow)' : 'none', background: i % 2 === 0 ? 'var(--parchment-deep)' : 'var(--parchment)' }}>
          <div style={{ width: 40, height: 40, border: '1.5px solid var(--ochre)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontFamily: "'Frank Ruhl Libre', serif", fontWeight: 900, fontSize: 18, color: 'var(--ochre)' }}>{s.step}</div>
          <div>
            <div style={{ fontFamily: "'Frank Ruhl Libre', serif", fontWeight: 500, fontSize: 20, marginBottom: 4 }}>{s.title}</div>
            <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 17, color: 'var(--ink-soft)', lineHeight: 1.6 }}>{s.desc}</div>
          </div>
        </div>
      ))}
    </div>
    ...
    <div>
      ...
      <div style={{ maxWidth: 820, margin: '0 auto', padding: '0 64px 100px' }}>
        ...
        <div ref={el => stepRefs.current[3] = el} style={{ paddingTop: 50, paddingBottom: 40, borderBottom: '1px solid var(--parchment-shadow)' }}>
          ...
          {session.study_method && <StudyMethodCard method={session.study_method} />}
        </div>
      ...
      {showModal && (
        <SessionCompletionModal
          ...
          onContinue={() => {
            setShowModal(false);
            if (session.id < 10) {
              const nextSession = C.weeks_data.flatMap(w => w.sessions).find(s => s.id === session.id + 1);
              const nextWeek = C.weeks_data.find(w => w.sessions.some(s => s.id === session.id + 1));
...
// Expose to window
Object.assign(window, { CourseLanding, WeekView, SessionView, SiteNav, Footer, COURSE_DATA, StudyMethodCard });
